...
class Address:
    ...
    def get_address(self):
        ...
        print('Address Format: 1234 street name, Mountain View, CA 94043')
        ...
        try:
            ...
            print('\nSorry. We couldn\'t find the latitude and the longitude coordinates for your address.\n')
            print('Please enter a valid address!')
            exit()

    @classmethod
    def get_cities(self):
        self.cities=input('\nEnter the cities where you want to find a restaurant separated by comma (Example: Sunnyvale, Cupertino, Mountain View): ')
    ...
    def address_filter(self):
        ...
        distance_pref=int(input('\nPlease enter the prefered distance from your address in miles( 5 or 10): '))
        self.radius=distance_pref
        
        try:
            if distance_pref not in [5,10]:
                print('\n[X] Please enter 5 or 10 miles.')
...
class Budget:
    ...
    @classmethod
    def get_budget(self):
        self.budget=input('\nEnter your budget preferences ($ ranking system): ')
        if self.budget not in ['$','$$','$$$','$$$$','$$$$$']:
            print('\n[X] Your input must be in $ ranking system (e.g. $$$).')
...
class Recommendations:
    ...
    def get_recommandations(self):
        ...
        if len(return_list)==0:
            ...
            print('\n\nThank you for using our application.')
            print('------------------------------------')
            choice=input('Do you wish to go again? (Y/N): ').lower()
            if choice == 'y':
                os.execl(sys.executable, sys.executable, *sys.argv)
            elif choice == 'n':
                print('\nThank you for using our application.\n')
...
if __name__ == "__main__":
    ...
    print('\n   Disclaimer! This is a demo version of our application built only for users who are located in Mountain View, CA')
    ...
    if area in ['N', 'n', 'No', 'no']:
        print('\nSorry. Our demo version is designed only for users who are located in Mountain View CA.\n')
        exit(0)
    ...
    while True:
        ...
        print('--------------------------------')    
        print('[1] - Your address: "{:s}"'.format(user.user_address.address))
        print('[2] - Cities to search: "{:s}"'.format(user.user_address.cities))
        print('[3] - Search radius: {:d}'.format(user.user_address.radius))
        print('[4] - Dietary preferences: "{:s}"'.format(user.user_dietary_pref.diet))
        print('[5] - Budget preferences: "{:s}"'.format(user.user_budget.budget))
        print('[6] - Rating preferences: {:d}'.format(user.user_rating_pref.rating))
        print('[7] - Food Type Preferences: "{:s}"'.format(user.user_food_pref.food_type))
        print('\n')
        print('[8] - Show my recommandations!')
        print('[0] - Exit')
